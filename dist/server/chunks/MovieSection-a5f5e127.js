import { c as create_ssr_component, d as compute_rest_props, j as createEventDispatcher, s as setContext, o as onDestroy, f as spread, h as escape_attribute_value, g as escape_object, k as add_attribute, t as tick, l as getContext, v as validate_component, e as escape, i as each$1 } from "./index-04ee162a.js";
import { I as Icon, b as Star } from "./Icon-3ba561f3.js";
const variables = {
  movieUrl: "https://api.themoviedb.org/3",
  movieToken: "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhNDk5NjNjMTVjM2VjYWFmNzY5NDViMTJkYWUxNmY3ZSIsInN1YiI6IjYyMmE0MmIzNzcwNzAwMDA2ZTJmZWM3NCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.NBnCnq2YR76te9fsphOLt352RHyhB3rekNu6xs2Pz9A"
};
...
const Rating = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"text-xs flex"}"><div class="${"flex space-x-0.5 px-1.5 py-1 bg-gray-light text-primary-dark rounded-xl font-medium"}">${validate_component(Icon, "Icon").$$render($$result, { src: Star, class: "w-3", solid: true }, {}, {})}
		<div>${escape(Math.ceil(value))}/10</div></div></div>`;
});
const MoviePreview = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { movie } = $$props;
  const getYear = (date) => {
  ...
  return `<a href="${"/" + escape(movie.id)}" class="${"bg-light dark:bg-gray-dark dark:text-gray-light"}"><div class="${"overflow-hidden relative group"}"><div class="${"absolute flex flex-col justify-between w-full h-full z-20 text-light"}"><div class="${"p-6"}">${validate_component(Rating, "Rating").$$render($$result, { value: movie.vote_average }, {}, {})}</div>
			<div class="${"p-6 flex flex-col space-y-1"}"><div class="${"sm:text-lg font-secondary font-bold sm:group-hover:text-2xl transition-all"}">${escape(movie.title)}</div>
				<div class="${"text-xs"}">${escape(getYear(movie.release_date))}</div></div></div>
		...
		<img class="${"object-cover object-bottom bg-light w-full h-56 sm:h-48 group-hover:scale-105 transition-all duration-200"}" src="${"https://image.tmdb.org/t/p/w500" + escape(movie.backdrop_path)}" alt="${"poster"}"></div></a>`;
});
var swiper_min = "";
const MovieSection = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let slidesPerView;
...
${validate_component(Swiper, "Swiper").$$render($$result, { spaceBetween: 10, slidesPerView }, {}, {
    default: () => {
      return `${each$1(movies2.results, (movie, index2) => {
        return `${validate_component(Swiper_slide, "SwiperSlide").$$render($$result, {}, {}, {
          default: () => {
            return `<div>${validate_component(MoviePreview, "MoviePreview").$$render($$result, { movie }, {}, {})}</div>
		`;
          }
        })}`;
      })}`;
...
export { MovieSection as M, Rating as R, Swiper as S, Swiper_slide as a, getDocument as g, movies as m, nextTick as n };
